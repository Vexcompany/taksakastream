...
function extractCards($) {
  const results = []
  $(".listupd .bs, .postbody .bs, .bsx").each((_, el) => {
    const $el  = $(el)
...
/**
 * getSchedule — BUG FIX: dump ALL text content of page, match day names anywhere
 */
async function getSchedule() {
  ...
  // Try several known schedule URL patterns on Anoboy
  const scheduleUrls = [
    "https://anoboy.be/jadwal-tayang/",
    "https://anoboy.be/jadwal/",
    "https://anoboy.be/schedule/",
    "https://anoboy.be/",          // homepage sometimes has schedule widget
  ]
  ...
  for (const schedUrl of scheduleUrls) {
    try {
      ...
      // ── Strategy 1: structured day containers ──
      const dayContainerSels = [
        ".schedule-day", ".jadwal-hari", ".hari", ".day-title",
        "h2", "h3", "h4", ".widget-title", ".bgtitle", ".ts-header",
      ]
      for (const sel of dayContainerSels) {
        $(sel).each((_, dayEl) => {
          const txt     = $(dayEl).text().trim().toLowerCase()
          const matched = dayOrder.find(d => txt.includes(d.toLowerCase()))
            || dayOrder.find(d => (dayMap[txt] || "") === d)
          if (!matched) return

          // Find sibling/child list items
          const $next = $(dayEl).nextUntil(sel)
          $next.find("li, .schedule-item, .anime-item, a").each((_, item) => {
            const $item = $(item)
            const title = $item.find("a").text().trim() || $item.text().trim()
            const url   = $item.find("a").attr("href") || $item.attr("href") || null
            const time  = $item.find(".time, .jam, .hour").text().trim() || null
            const ep    = $item.find(".ep, .episode").text().trim() || null
            if (title && title.length > 2) {
              schedule[matched].push({ title, url, time, episode: ep })
            }
          })

          // Also check direct children
          $(dayEl).find("li, .anime-item").each((_, item) => {
            const $item = $(item)
            const title = $item.find("a").text().trim() || $item.text().trim()
            const url   = $item.find("a").attr("href") || null
            const time  = $item.find(".time, .jam").text().trim() || null
            if (title && title.length > 2) {
              schedule[matched].push({ title, url, time, episode: null })
            }
          })
        })
      }
      ...
      $("table tr, .tb-schedule tr, .schedule-table tr").each((_, row) => {
        const cells = $(row).find("td")
        if (cells.length < 2) return
        const dayTxt  = $(cells[0]).text().trim().toLowerCase()
        const matched = dayOrder.find(d => dayTxt.includes(d.toLowerCase()))
        if (!matched) return
        const title = $(cells[1]).text().trim()
        const url   = $(cells[1]).find("a").attr("href") || null
        const time  = $(cells[cells.length - 1]).text().trim() || null
        if (title && title.length > 2) schedule[matched].push({ title, url, time, episode: null })
      })

      // ── Strategy 3: widget / sidebar with day headings ──
      $(".widget, .sidebar-widget, .widget_text, #sidebar").find("ul, ol").each((_, list) => {
        // Look for a preceding heading that names a day
        const $prev = $(list).prev()
        const headTxt = $prev.text().trim().toLowerCase()
        const matched = dayOrder.find(d => headTxt.includes(d.toLowerCase()))
        if (!matched) return
        $(list).find("li").each((_, li) => {
          const title = $(li).find("a").text().trim() || $(li).text().trim()
          const url   = $(li).find("a").attr("href") || null
          if (title && title.length > 2) schedule[matched].push({ title, url, time: null, episode: null })
        })
      })

      // If at least 1 day has data, stop trying URLs
