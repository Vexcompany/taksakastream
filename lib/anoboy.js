...
// ─────────────────────────────────────────────
// GENRE SLUG MAP (Anoboy genre URL slugs)
// ─────────────────────────────────────────────
const GENRE_SLUGS = {
  "action":       "action",
  "adventure":    "adventure",
 ...
 * Scrape Anoboy genre page — returns full anime list with thumbnails.
 */
async function getByGenre(genre, page = 1) {
  ...
  const url  = page > 1
    ? `https://anoboy.be/genre/${slug}/page/${page}/`
    : `https://anoboy.be/genre/${slug}/`
  ...
  try {
    ...
    $(".listupd .bs").each((_, el) => {
      ...
      const thumb = $el.find("img").attr("src") || $el.find("img").attr("data-src") || null
      const status= $el.find(".status").text().trim() || null
      const type  = $el.find(".typez").text().trim() || null
      const ep    = $el.find(".epx").text().trim() || null
      if (aUrl && title) results.push({ title, url: aUrl, thumbnail: thumb, status, type, episode: ep })
    })

    // Detect total pages
    const lastPage = parseInt($(".pagination .last a").attr("href")?.match(/page\/(\d+)/)?.[1] || "1")

    return {
      code: 200, timestamp: Date.now(),
      data: { genre, slug, page, total_pages: lastPage, total_results: results.length, results }
    }
  } catch (err) {
    throw new Error(`Gagal mengambil genre ${genre}: ${err.message}`)
  }
}

/**
 * getSchedule()
 * Scrape Anoboy jadwal tayang (airing schedule) — grouped by day.
 */
async function getSchedule() {
  const client = buildClient("https://anoboy.be/")
  try {
    const { data: html } = await client.get("https://anoboy.be/jadwal-tayang/")
    const $ = cheerio.load(html)

    const schedule = {}
    const dayOrder = ["Senin","Selasa","Rabu","Kamis","Jumat","Sabtu","Minggu"]

    // Anoboy jadwal structure: .schedule-nav days, .schedule-list items
    $(".schedule-day, .jadwal-hari, h2.day-title, .hari").each((_, dayEl) => {
      const dayName = $(dayEl).text().trim()
      const matched = dayOrder.find(d => dayName.includes(d))
      if (!matched) return

      const items = []
      $(dayEl).nextUntil(".schedule-day, .jadwal-hari, h2.day-title, .hari").find(".schedule-item, li, .animelist-item").each((_, item) => {
        const title = $(item).find("a, .title").first().text().trim()
        const url   = $(item).find("a").first().attr("href") || null
        const ep    = $(item).find(".episode, .ep").text().trim() || null
        const time  = $(item).find(".time, .jam").text().trim() || null
        if (title) items.push({ title, url, episode: ep, time })
      })

      if (items.length) schedule[matched] = items
    })

    // Fallback: try table-based layout
    if (!Object.keys(schedule).length) {
      $("table tr, .tb-schedule tr").each((_, row) => {
        const cells = $(row).find("td")
        if (cells.length < 2) return
        const day   = $(cells[0]).text().trim()
        const matched = dayOrder.find(d => day.includes(d))
        if (!matched) return
        if (!schedule[matched]) schedule[matched] = []
        const title = $(cells[1]).text().trim()
        const url   = $(cells[1]).find("a").attr("href") || null
        const ep    = $(cells[2])?.text().trim() || null
        const time  = $(cells[3])?.text().trim() || null
        if (title) schedule[matched].push({ title, url, episode: ep, time })
      })
    }

    // Fallback 2: parse sidebar/widget schedule
    if (!Object.keys(schedule).length) {
      $(".widget-schedule, .jadwal-widget, #jadwal").each((_, widget) => {
        $(widget).find("h3, h4, .day").each((_, dEl) => {
          const dText  = $(dEl).text().trim()
          const matched = dayOrder.find(d => dText.includes(d))
          if (!matched) return
          if (!schedule[matched]) schedule[matched] = []
          $(dEl).nextUntil("h3,h4,.day").filter("li, .item").each((_, li) => {
            const title = $(li).find("a").text().trim() || $(li).text().trim()
            const url   = $(li).find("a").attr("href") || null
            const time  = $(li).find(".time").text().trim() || null
            if (title) schedule[matched].push({ title, url, episode: null, time })
          })
        })
      })
    }

    // Ensure all days exist (even empty)
    dayOrder.forEach(d => { if (!schedule[d]) schedule[d] = [] })

    return { code: 200, timestamp: Date.now(), data: { schedule, day_order: dayOrder } }
  } catch (err) {
    throw new Error(`Gagal mengambil jadwal: ${err.message}`)
  }
}

/**
 * getLatest(page?)
 * Scrape latest/recent anime updates from Anoboy homepage.
 */
async function getLatest(page = 1) {
  ...
  try {
    ...
    $(".listupd .bs, .postbody .bs").each((_, el) => {
      const $el   = $(el)
      const $a    = $el.find("a")
      const aUrl  = $a.attr("href")
      const title = $a.find(".tt").text().trim() || $a.attr("title") || ""
    ...
    return { code: 200, timestamp: Date.now(), data: { page, total_results: results.length, results } }
  } catch (err) {
    throw new Error(`Gagal mengambil latest: ${err.message}`)
  }
}
